    ...
    if ($username != '' && $password != '') {
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                ...
                $_SESSION['id_jajaran'] = $row['id_jajaran'];
                $_SESSION['jajaran'] = $row['jajaran'];
                $_SESSION['user_login'] = $row['login'];
                $_SESSION['user_name'] = $row['real_name'];
                $_SESSION['user_level'] = $row['level'];
                header('Location:index.php?v=0.4');
            } else {
                ?>
                <script>alert('User atau Password Salah');</script>
                <?php
            }
        } else {
            ?>
            <script>alert('Ada kesalahan pada system');</script>
            <?php
        }
        $mysqli->close();
    } else {
        ?>
        <script>alert('User atau Password Tidak Boleh Kosong');</script>
        <?php
    }
}
?>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Quick Count System</title>
        <style>
            /* Basics */
            html, body {
                width: 100%;
    ...
    <body>
        ...
        <div id="container">
            <form method="post">
                <label for="username">Username:</label> <input type="text" id="username" name="username"></input>
